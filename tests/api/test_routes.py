...
def test_health_and_chart_routes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(create_app(settings), raise_server_exceptions=False) as client:
        health = client.get(
            "/api/v1/health", headers={"X-Request-ID": "api-route-test-request"}
        )
    ...
    ziping_passage = next(
        item
        for item in originals.json()["passages"]
        if item["id"].startswith("ziping-zhenquan-")
    )
    ...
    assert not any(
        item["review_status"] == "machine_verified"
        for item in reviewed_chat.json()["evidence"]
    )
    assert preview_chat.status_code == 200
    assert any(
        item["review_status"] == "machine_verified"
        for item in preview_chat.json()["evidence"]
    )
    ...
    assert (
        internal_chart_error.headers["X-Request-ID"]
        == internal_chart_error.json()["request_id"]
    )
    assert internal_chart_error.headers["Access-Control-Allow-Origin"] == (
        "http://localhost:3000"
    )
